 ...
 */
final class Version20250925151857 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__media AS SELECT id, original_name, mime_type, size, path, created_at FROM media');
        $this->addSql('DROP TABLE media');
        $this->addSql('CREATE TABLE media (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, original_name VARCHAR(255) DEFAULT NULL, mime_type VARCHAR(100) DEFAULT NULL, size INTEGER DEFAULT NULL, path VARCHAR(500) DEFAULT NULL, created_at DATETIME NOT NULL --(DC2Type:datetime_immutable)
        , file_name VARCHAR(255) DEFAULT NULL, alt VARCHAR(255) DEFAULT NULL, extension VARCHAR(255) DEFAULT NULL)');
        $this->addSql('INSERT INTO media (id, original_name, mime_type, size, path, created_at) SELECT id, original_name, mime_type, size, path, created_at FROM __temp__media');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__media AS SELECT id, original_name, mime_type, size, path, created_at FROM media');
        $this->addSql('DROP TABLE media');
        $this->addSql('CREATE TABLE media (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, original_name VARCHAR(255) NOT NULL, mime_type VARCHAR(100) NOT NULL, size INTEGER NOT NULL, path VARCHAR(500) NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL, filename VARCHAR(255) NOT NULL, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL)');
        $this->addSql('INSERT INTO media (id, original_name, mime_type, size, path, created_at) SELECT id, original_name, mime_type, size, path, created_at FROM __temp__media');
        $this->addSql('DROP TABLE __temp__media');
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, email, roles, password, first_name, last_name, is_active, created_at, updated_at, last_login_at, phone, client_type, monthly_income, monthly_charges, employment_status, employer, employment_start_date, business_name, business_registration, business_years, annual_revenue, is_account_verified, verified_at FROM user');
        $this->addSql('DROP TABLE user');
